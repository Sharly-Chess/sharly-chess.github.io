...
| **Name** | A more user-friendly name, used for display purposes (e.g. on [Screens]({{ site.baseurl }}/screens)). |
...
| **Default directory for Papi files** | Currently, most tournament data is stored in a _[Papi]({{ site.baseurl }}/relation-to-papi)_ file. This field allows you to define a default location for these files, and can be overridden on a per-tournament basis. |
...
| **Alert message color and text** | If you’re using [Screens]({{ site.baseurl }}/screens), you can define a scrolling message to appear across the bottom. This default message can also be overridden on a per-screen basis. |
