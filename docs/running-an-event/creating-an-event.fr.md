...
| **Visibilité** | Pertinent uniquement si vous utilisez d'autres appareils sur votre réseau pour afficher des [Écrans]({{ site.baseurl }}/ecrans). Seuls les événements marqués comme *publics* seront accessibles depuis ces appareils. |
...
| **Nom** | Un nom plus lisible, utilisé à des fins d’affichage (par exemple sur les [Écrans]({{ site.baseurl }}/ecrans)). |
...
| **Dossier par défaut pour les fichiers Papi** | Actuellement, la plupart des données de tournoi sont stockées dans un fichier _[Papi]({{ site.baseurl }}/relation-avec-papi)_. Ce champ permet de définir un emplacement par défaut pour ces fichiers, pouvant être remplacé au niveau de chaque tournoi. |
...
| **Message d’alerte et couleur** | Si vous utilisez des [Écrans]({{ site.baseurl }}/ecrans), vous pouvez définir un message défilant affiché en bas de l’écran. Ce message par défaut peut également être modifié écran par écran. |
